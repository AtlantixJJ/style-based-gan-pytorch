...
for i in range(args.n_total):
    ...
    latent.copy_(original_latents[i])
    ...
    utils.plot_dic(record, "label edit loss", f"{outdir}/{optimizer}_s{args.seed}_n{args.n_iter}_k{args.kl_coef}_{i:02d}_loss.png")
    ...
    snaps = torch.cat([utils.bu(r, 256) for r in res])
    vutils.save_image(snaps, f"{outdir}/{optimizer}_n{args.n_iter}_k{args.kl_coef}_{i:02d}_snapshot.png", nrow=4)

    # save optimization process
    np.save(
        f"{outdir}/{optimizer}_s{args.seed}_n{args.n_iter}_k{args.kl_coef}_{i:02d}_latents.npy",
