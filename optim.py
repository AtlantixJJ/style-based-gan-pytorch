...
class Temperture(object): # for 10 iters
    def __init__(self, T0=0.5, alpha=0.2):
        self.T0 = T0
        self.alpha = alpha
...
def edit_label_stroke(model, latent, noises, label_stroke, label_mask,
    ...
    for i in tqdm(range(n_iter)):
        ...
        if total_diff < 1:
            ...
            # enhance target area, suppress other
            diff_mask = diff_mask * 0.25
            diff_mask = diff_mask + 2 * label_mask
        ...
        loss = mseloss + 0.1 * celoss
        grad = torch.autograd.grad(loss, latent)[0]
        grad = grad + torch.randn_like(grad) * T(i)
